...
function getExportedTypes(filePath) {
    ...
    while ((match = interfaceTypeRegex.exec(content)) !== null && match.length > 1) {
...
try {
    console.log("Running script (generateSchemas.js) to auto-generate JSON schemas from TypeScript model files...");
    ...
    console.log(`✅ Schema generation complete! (${((endTime - startTime) / 1000).toFixed(2)}s)`);
    console.log(`📊 Generated ${totalSchemas} schemas in ${generatedFiles.length} file(s)`);
